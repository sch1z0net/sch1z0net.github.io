let bitrv2 = function (n, ipBuffer, ipOffset, aBuffer) {
	let j, j1, k, k1, l, m;
	let xr, xi, yr, yi;
...
let bitrv2conj = function (n, ipBuffer, ipOffset, aBuffer) {
	let j, j1, k, k1, l, m;
	let xr, xi, yr, yi;
...
let cftfsub = function (n, aBuffer, wBuffer) {
	let j, j1, j2, j3, l;
	let x0r, x0i, x1r, x1i, x2r, x2i, x3r, x3i;
...
let cftbsub = function (n, aBuffer, wBuffer) {
	let j, j1, j2, j3, l;
	let x0r, x0i, x1r, x1i, x2r, x2i, x3r, x3i;
...
let rftfsub = function (n, aBuffer, nc, cBuffer, cOffset) {
	let j, k, kk;
...
let rftbsub = function (n, aBuffer, nc, cBuffer, cOffset) {
	let j, k, kk;
	let wkr, wki, xr, xi, yr, yi;
...
let makewt = function (nw, ipBuffer, wBuffer) {
	let nwh;
	let delta;
	let x;
	let y;
...
let makect = function (nc, ipBuffer, cBuffer, cOffset) {
	let j;
	let nch;
	let delta;
...
let rdft = function (n, dir, aBuffer, ipBuffer, wBuffer) {
	...
	if (dir === DIRECTION.FORWARDS) {
		if (n > 4) {
			...
			rftfsub(n, aBuffer, nc, wBuffer, nw);
		} else if (n === 4) {
			cftfsub(n, aBuffer, wBuffer);
		}
		const xi = a[0] - a[1];
		...
		a[0] -= a[1];
		if (n > 4) {
			rftbsub(n, aBuffer, nc, wBuffer, nw);
			bitrv2(n, ipBuffer, 2, aBuffer);
...
let cdft = function (n, dir, aBuffer, ipBuffer, wBuffer) {
	if (n > 4) {
		if (dir === DIRECTION.FORWARDS) {
			bitrv2(n, ipBuffer, 2, aBuffer);
			cftfsub(n, aBuffer, wBuffer);
		} else {
			bitrv2conj(n, ipBuffer, 2, aBuffer);
			cftbsub(n, aBuffer, wBuffer);
		}
	} else if (n === 4) {
		cftfsub(n, aBuffer, wBuffer);
...
class Ooura {
	...
	fftReal(dataBuffer, reBuffer, imBuffer) {
		...
		rdft(this.size, DIRECTION.FORWARDS, this.internal.buffer, this.ip.buffer, this.w.buffer);
	...
	ifftReal(dataBuffer, reBuffer, imBuffer) {
		...
		rdft(this.size, DIRECTION.BACKWARDS, this.internal.buffer, this.ip.buffer, this.w.buffer);
	...
	xfftComplex(direction, reIpBuffer, imIpBuffer, reOpBuffer, imOpBuffer) {
		...
		cdft(this.size, direction, this.internal.buffer, this.ip.buffer, this.w.buffer);
	...
	fftComplex(reIpBuffer, imIpBuffer, reOpBuffer, imOpBuffer) {
		this.xfftComplex(DIRECTION.FORWARDS, reIpBuffer, imIpBuffer, reOpBuffer, imOpBuffer);
	}

	ifftComplex(reIpBuffer, imIpBuffer, reOpBuffer, imOpBuffer) {
		this.xfftComplex(DIRECTION.BACKWARDS, reIpBuffer, imIpBuffer, reOpBuffer, imOpBuffer);
		const reOp = new Float64Array(reOpBuffer);
		const imOp = new Float64Array(imOpBuffer);
	...
	fftInPlaceReal(dataBuffer) {
		rdft(this.size, DIRECTION.FORWARDS, dataBuffer, this.ip.buffer, this.w.buffer);
	}

	fftInPlaceComplex(dataBuffer) {
		cdft(this.size, DIRECTION.FORWARDS, dataBuffer, this.ip.buffer, this.w.buffer);
	}

	ifftInPlaceReal(dataBuffer) {
		rdft(this.size, DIRECTION.BACKWARDS, dataBuffer, this.ip.buffer, this.w.buffer);
	}

	ifftInPlaceComplex(dataBuffer) {
		cdft(this.size, DIRECTION.BACKWARDS, dataBuffer, this.ip.buffer, this.w.buffer);
	}
