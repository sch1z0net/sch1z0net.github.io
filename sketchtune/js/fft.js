...
function fftComplexInPlace_seq_4(out) {
    ...
    for(let p = 0; p < (bits>>1); p++){
        ...
        for(let i = 0; i < N; i+=4){
            ...
            x0Re = out[(s + k + d*0)*2+0]; x0Im = out[(s + k + d*0)*2+1];  //x0 
            x1Re = out[(s + k + d*1)*2+0]; x1Im = out[(s + k + d*1)*2+1];  //x1
            x2Re = out[(s + k + d*2)*2+0]; x2Im = out[(s + k + d*2)*2+1];  //x2 
            x3Re = out[(s + k + d*3)*2+0]; x3Im = out[(s + k + d*3)*2+1];  //x3
