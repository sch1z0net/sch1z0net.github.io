...
function fftComplexInPlace_seq_4(out) {
    ...
    tRe_2e  = ____F[14+(4*2+0)]; 
    tIm_2e  = ____F[14+(4*2+1)];
...
/*
    for(let i_ = 0; i_ < tsq; i_+=4){
          ...
          x0aRe = out[(i+ 0)*2+0]; x0aIm = out[(i+ 0)*2+1];
          x1aRe = out[(i+ 4)*2+0]; x1aIm = out[(i+ 4)*2+1];
          x2aRe = out[(i+ 8)*2+0]; x2aIm = out[(i+ 8)*2+1];
          x3aRe = out[(i+12)*2+0]; x3aIm = out[(i+12)*2+1];
          ...
          x1cRe = out[(i+ 6)*2+0]; x1cIm = out[(i+ 6)*2+1];
          x2cRe = out[(i+10)*2+0]; x2cIm = out[(i+10)*2+1];
          x3cRe = out[(i+14)*2+0]; x3cIm = out[(i+14)*2+1];

          x0dRe = out[(i+ 3)*2+0]; x0dIm = out[(i+ 3)*2+1];
          x1dRe = out[(i+ 7)*2+0]; x1dIm = out[(i+ 7)*2+1];
          x2dRe = out[(i+11)*2+0]; x2dIm = out[(i+11)*2+1];
          x3dRe = out[(i+15)*2+0]; x3dIm = out[(i+15)*2+1];
          ...
          out[( 8+i)*2+0] = xM0ReA - ((xM2ReA)*tRe_2a - ((xM2ImA)*tIm_2a));
          out[( 8+i)*2+1] = xM0ImA - ((xM2ReA)*tIm_2a + ((xM2ImA)*tRe_2a)); 
          out[( 9+i)*2+0] = xM1ReB - ((xM3ReB)*tRe_2b - ((xM3ImB)*tIm_2b));
          out[( 9+i)*2+1] = xM1ImB - ((xM3ReB)*tIm_2b + ((xM3ImB)*tRe_2b)); 
          out[(10+i)*2+0] = xM0ReC - ((xM2ReC)*tRe_2c - ((xM2ImC)*tIm_2c));
          out[(10+i)*2+1] = xM0ImC - ((xM2ReC)*tIm_2c + ((xM2ImC)*tRe_2c));
          out[(11+i)*2+0] = xM1ReD - ((xM3ReD)*tRe_2d - ((xM3ImD)*tIm_2d));
          out[(11+i)*2+1] = xM1ImD - ((xM3ReD)*tIm_2d + ((xM3ImD)*tRe_2d));

          out[(12+i)*2+0] = xM0ReA - ((xM2ReA)*tRe_2e - ((xM2ImA)*tIm_2e));
          out[(12+i)*2+1] = xM0ImA - ((xM2ReA)*tIm_2e + ((xM2ImA)*tRe_2e)); 
          out[(13+i)*2+0] = xM1ReB - ((xM3ReB)*tRe_2f - ((xM3ImB)*tIm_2f));
          out[(13+i)*2+1] = xM1ImB - ((xM3ReB)*tIm_2f + ((xM3ImB)*tRe_2f)); 
          out[(14+i)*2+0] = xM0ReC - ((xM2ReC)*tRe_2g - ((xM2ImC)*tIm_2g));
          out[(14+i)*2+1] = xM0ImC - ((xM2ReC)*tIm_2g + ((xM2ImC)*tRe_2g));
    ...
    */
    ...
    for (let i_ = 0; i_ < tsq; i_ += 4) {
        ...
        let ReVals = new Array(16);
        let ImVals = new Array(16);

        for (let j = 0; j < 16; j++) {
            ReVals[j] = out[(i + j) * 2];
            ImVals[j] = out[(i + j) * 2 + 1];
        }
        ...
        for (let j = 0; j < 4; j++) {
            ...
            let x1Re = ReVals[j + 4];
            let x2Re = ReVals[j + 8];
            let x3Re = ReVals[j + 12];

            let x0Im = ImVals[j];
            let x1Im = ImVals[j + 4];
            let x2Im = ImVals[j + 8];
            let x3Im = ImVals[j + 12];

            let Tx0Re = x1Re * tRe_1a - x1Im * tIm_1a;
            let Tx0Im = x1Re * tIm_1a + x1Im * tRe_1a;
            let Tx2Re = x3Re * tRe_1a - x3Im * tIm_1a;
            let Tx2Im = x3Re * tIm_1a + x3Im * tRe_1a;

            let Tx1Re = x1Re * tRe_1b - x1Im * tIm_1b;
            let Tx1Im = x1Re * tIm_1b + x1Im * tRe_1b;
            let Tx3Re = x3Re * tRe_1b - x3Im * tIm_1b;
            let Tx3Im = x3Re * tIm_1b + x3Im * tRe_1b;

            let xM0Re = x0Re + Tx0Re;
            let xM0Im = x0Im + Tx0Im;
            let xM2Re = x2Re + Tx2Re;
            let xM2Im = x2Im + Tx2Im;

            let xM1Re = x0Re + Tx1Re;
            let xM1Im = x0Im + Tx1Im;
            let xM3Re = x2Re + Tx3Re;
            let xM3Im = x2Im + Tx3Im;

            out[(j + i) * 2]          = xM0Re + (xM2Re * tRe_2a - xM2Im * tIm_2a);
            out[(j + i) * 2 + 1]      = xM0Im + (xM2Re * tIm_2a + xM2Im * tRe_2a);
            out[(j + i + 4) * 2]      = xM1Re + (xM3Re * tRe_2b - xM3Im * tIm_2b);
            out[(j + i + 4) * 2 + 1]  = xM1Im + (xM3Re * tIm_2b + xM3Im * tRe_2b);
            out[(j + i + 8) * 2]      = xM0Re + (xM2Re * tRe_2c - xM2Im * tIm_2c);
            out[(j + i + 8) * 2 + 1]  = xM0Im + (xM2Re * tIm_2c + xM2Im * tRe_2c);
            out[(j + i + 12) * 2]     = xM1Re + (xM3Re * tRe_2d - xM3Im * tIm_2d);
            out[(j + i + 12) * 2 + 1] = xM1Im + (xM3Re * tIm_2d + xM3Im * tRe_2d);
            ...
            out[(j + i + 16) * 2]     = xM0Re - (xM2Re * tRe_2a - xM2Im * tIm_2a);
            out[(j + i + 16) * 2 + 1] = xM0Im - (xM2Re * tIm_2a + xM2Im * tRe_2a);
            out[(j + i + 20) * 2]     = xM1Re - (xM3Re * tRe_2b - xM3Im * tIm_2b);
            out[(j + i + 20) * 2 + 1] = xM1Im - (xM3Re * tIm_2b + xM3Im * tRe_2b);
            out[(j + i + 24) * 2]     = xM0Re - (xM2Re * tRe_2c - xM2Im * tIm_2c);
            out[(j + i + 24) * 2 + 1] = xM0Im - (xM2Re * tIm_2c + xM2Im * tRe_2c);
            out[(j + i + 28) * 2]     = xM1Re - (xM3Re * tRe_2d - xM3Im * tIm_2d);
            out[(j + i + 28) * 2 + 1] = xM1Im - (xM3Re * tIm_2d + xM3Im * tRe_2d);
        }
    }
